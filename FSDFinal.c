...
/**
  ...
  * Author: Thomas Whitford, Contact: twhitford2224@gmail.com
  * This is the code Final Frequency Support Device with speed feedback
  ******************************************************************************
  * @attention
  *
...
{
  /* USER CODE BEGIN 1 */
	double motor_speed = 176;//*0.1047197551;  //will be read by ADC from motor speed measurement
	double target_speed = 176;//*0.1047197551; //will be set by PC / Potentiometer (equivalent to thermostat)
	double torque_ref = 8.30; //calculated by calcTORQUE function
	double torque_rated = 11.40; //motor constant
	uint32_t Data_Tx = 0;
	...
	float t = 0;
	float setpoint = 1500 * 0.1047197551;
	float measurement = 0;
  /* USER CODE END 1 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (TorqueFlag == 1){
		  Freq = gFreqAvg;
		  read_SDO(canopenNodeSTM32.canOpenStack->SDOclient, 0x01, 0x2003, 0x02	, &readSpeed, 4, &amount_Read);
		  measurement = ((float)readSpeed)*0.01047197551f;
		  torque_ref = calcTORQUE(Freq, measurement, setpoint); //calculate Torque reference value in Nm
		  ...
		  HAL_GPIO_TogglePin(GPIOC,GPIO_PIN_7);
	  }else if (TorqueFlag == 2){ //speed loop operation
		  //Read speed
		  Freq = gFreqAvg;
		  sprintf(msg, "%.3f\n", Freq);
		  HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);
          ...
          Data_Tx = calcCANTrefvalue(torque_calc_PID,torque_rated);
          write_SDO(canopenNodeSTM32.canOpenStack->SDOclient, 0x01, 0x2004, 0x08, &Data_Tx, 4);
          canopen_app_process();
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
    ...
    if (oldstate != state){
    	//edge detection
    	uint8_t outmsg = state;
    	//startFlag = 1;
    	HAL_GPIO_TogglePin(GPIOA,GPIO_PIN_9);
    	TimVals[indx] = __HAL_TIM_GET_COUNTER(&htim6);
    	if(indx == 2){
    		indx = 0;
    		if(TimVals[2] > TimVals[0]){
    			difference = TimVals[2] - TimVals[0];

    		}else{
    			 difference = (0xffffffff - TimVals[0]) + TimVals[2];

    		}
    		float frequency = 1000000.0f/(float)difference;

    		if (abs(frequency) < 52 && abs(frequency) > 48){

    			if (fIndex < 10){
    				ftotal = ftotal + frequency;
    				fIndex++;
...
double calcTORQUE(double network_freq, double motor_speed, double target_speed){
	...
	if (network_freq <= 49.52){
		...
		if (network_freq <= 49.82){
		km= 1- ((49.82-freq_1)*8.571428571)- ((49.75-freq_2)*0.8695652174); //calculate km (percentage of initial motor power)
		}else {
			km= 1;
		}
		torque_ref_calc = km*(fan_k*motor_speed*motor_speed); //calculate torque reference in Nm
	}
